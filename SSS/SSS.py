#!/usr/bin/env python3
import re
import sys

P1 = ""
P2 = ""
P3 = ""

def norm_hex(h):
    # remove whitespace/newlines if pasted with formatting
    h = ''.join(h.split())
    # if odd length, left-pad with '0'
    if len(h) % 2 == 1:
        h = '0' + h
    return h

P1 = norm_hex(P1)
P2 = norm_hex(P2)
P3 = norm_hex(P3)

b1 = bytes.fromhex(P1)
b2 = bytes.fromhex(P2)
b3 = bytes.fromhex(P3)

# Ensure same length by truncating to shortest
L = min(len(b1), len(b2), len(b3))
b1, b2, b3 = b1[:L], b2[:L], b3[:L]

xor = bytes(a ^ b ^ c for a, b, c in zip(b1, b2, b3))

print("XOR bytes length:", len(xor))
print("XOR first 64 bytes (hex):", xor[:64].hex())

tried = {}
for enc in ("utf-16le", "utf-8", "latin-1", "utf-16be"):
    try:
        tried[enc] = xor.decode(enc)
    except Exception as e:
        tried[enc] = f"<decode error: {e}>"

for enc, dec in tried.items():
    print("\n--- decode:", enc)
    print(dec[:300])

s = tried.get("utf-16le", "")
if not s.startswith("<decode error"):
    rev = s[::-1]
    m1 = re.search(r'flag\{.*?\}', s, re.IGNORECASE)
    m2 = re.search(r'\{[0-9a-fA-F]{20,}\}', s)
    m3 = re.search(r'flag\{.*?\}', rev, re.IGNORECASE)
    m4 = re.search(r'\{[0-9a-fA-F]{20,}\}', rev)

    for i, m in enumerate((m1, m2, m3, m4), start=1):
        if m:
            print(f"\nFound pattern (case {i}):", m.group(0))
            sys.exit(0)
